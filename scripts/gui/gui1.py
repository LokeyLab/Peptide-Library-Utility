...
def single_peptide_creator(root, console_list):
    ...
    peptide_create_button = h.tk.Button(single_peptide_main_frame, text="Create Peptide",
                                        command=h.gui_functions.single_peptide_create_button(
                                            single_peptide_window,
                                                                                                       console_list,
                                                                                                       h.SUBUNIT_LIBRARY,
                                                                                                       h.SUBUNIT_LIST,
...
def peptide_library_creator(root, console_list, output_list, subunit_list, subunit_count):
    ...
    cheminformatic_options = [
        "Exact Mass", "ALogP", "SLogP", "SASA", "TPSA", "McGowan Volume",
        "# Atoms", "# Heteratoms", "# Amide Bonds", "# Rings", "# Aromatic Rings",
    ...
    h.tk.Label(file_io_frame, text="Input / Output").grid(column=0, columnspan=1,
                                                                row=0, rowspan=1,
                                                                padx=10, pady=10,
                                                                sticky='w')
...
def library_cheminformatics(root, console_list, output_list):
    ...
    file_types = ["Exact MW", "ALogP", "SLogP", "SASA", "TPSA", "McGowan Volume", "# Atoms",
                  "# Heteratoms", "# Amide Bonds",
...
def plotting(root, console_list, output_list):
    ...
    cheminformatic_options = ["Exact Mass", "ALogP", "SASA", "TPSA", "McGowan Volume",
...
def subunit_library_export(root, console_list, output_list, subunit_list,
                           subunit_count):
    console_list.insert("end", "Subunit Library Exporter Started.")
    console_list.insert("end", "")
    console_list.see("end")
    ...
    subunit_library_export_window, \
        subunit_library_export_main_frame, \
        subunit_library_export_progress_bar_text = create_window(root, 1080,
                                                                 120, 10, 10,
                                                                 "Subunit "
                                                                 "library "
                                                                 "Exporter")

    def subunit_library_export_exit_button():
        subunit_library_export_window.destroy()
        console_list.insert("end", "Subunit Library Exporter Exited.")
        console_list.insert("end", "")
        console_list.see("end")

    h.tk.Button(subunit_library_export_main_frame, text="Exit",
                command=subunit_library_export_exit_button).grid(column=1,
                                                                 columnspan=1,
                                                   row=0, rowspan=1, pady=10,
                                                   sticky="ne")

    output_frame = h.tk.LabelFrame(subunit_library_export_main_frame)
    output_frame.grid(column=0, columnspan=2, row=3, rowspan=1,
                         sticky='nsew')
    ...
    h.tk.Label(output_frame, textvariable=output_file_label_text).grid(
    ...
    def output_file_prompt():
        output_dir_name = h.filedialog.askdirectory(
            initialdir="your directory path",
            title="Output Folder")
        output_file_label_text.set(output_dir_name)
        h.OUTPUT_DIR = str(output_dir_name)

    h.tk.Button(output_frame, text="Output Folder",
                command=output_file_prompt).grid(column=1, columnspan=1,
                                                 row=0, rowspan=1,
                                                 padx=10, pady=10,
                                                 sticky="e")
    def export_data():
        ...
        df = h.utilities.subunits_to_dataframe()
        h.utilities.subunits_to_csv(df)

        subunit_library_export_window.destroy()

    h.tk.Button(subunit_library_export_main_frame, text="Export",
                command=export_data).grid(column=0, columnspan=1,
                                        row=0, rowspan=1,
                                        pady=10, padx=10,
                                        sticky="ne")
...
def create_main_window_main_menu(root, main_frame, console_list, output_list,
    ...
    h.tk.Button(main_menu_frame, text="Export Subunit Library",
                command=export_subunit_library, width=20).grid(column=0, columnspan=1,
                                    row=6, rowspan=1, padx=10, pady=10,
                                                  sticky="w")
...
def ui_loop():
    ...
    console_list.insert("end", "Welcome to Peptide Library Utility 0.2.1!")
    console_list.insert("end", "")

    console_list.insert("end", "****************Patch Notes****************")
    console_list.insert("end", "- McGowan Volume added for molecules")
    console_list.insert("end", "   containing atoms in periods 1-6.")
